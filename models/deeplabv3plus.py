...
import models.resnet101 as resnet101
...
class DeepLabV3plus(nn.Module):
    def __init__(self, band, class_num, bn_momentum=0.01):
        ...
        self.name = 'deeplabv3+'
        self.Resnet101 = resnet101.get_resnet101(band, dilation=[1, 1, 1, 2], bn_momentum=bn_momentum, is_fpn=True)
...
class DeepLabV3plusLAM(nn.Module):
    def __init__(self, band, class_num, bn_momentum=0.01):
        super(DeepLabV3plusLAM, self).__init__()

        self.name = 'deepLabv3plusLAM+'
        self.Resnet101 = resnet101.get_resnet101(band, dilation=[1, 1, 1, 2], bn_momentum=bn_momentum, is_fpn=True)
        self.head = Head(class_num, norm_layer=nn.BatchNorm2d)
        self.PLA = PositionLinearAttention(class_num)
    ...
    def forward(self, input):
        ...
        output = self.PLA(output)
        output = self.CLA(output)
...
def main():
    ...
    net = DeepLabV3plus(3, class_num=num_classes)
    out = net(x)
    print(out.shape)

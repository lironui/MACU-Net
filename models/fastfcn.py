...
class Head(nn.Module):
    def __init__(self, classify_classes, attention=False, norm_layer=nn.BatchNorm2d, bn_momentum=0.0003):
        ...
        self.PLA = PositionLinearAttention(256)
...
class FastFCN(nn.Module):
    def __init__(self, band, class_num, bn_momentum=0.01):
        ...
        self.name = 'FastFCN'
        self.Resnet101 = resnet.get_resnet101(band, dilation=[1, 1, 1, 2], bn_momentum=bn_momentum, is_fpn=True)
...
class FastFCNLAM(nn.Module):
    def __init__(self, band, class_num, bn_momentum=0.01):
        super(FastFCNLAM, self).__init__()
        self.name = 'FastFCNLAM'
        self.Resnet101 = resnet.get_resnet101(band, dilation=[1, 1, 1, 2], bn_momentum=bn_momentum, is_fpn=True)
        self.jpu = JPU([512, 1024, 2048], width=512, norm_layer=nn.BatchNorm2d)
        self.head = Head(class_num, norm_layer=nn.BatchNorm2d)
